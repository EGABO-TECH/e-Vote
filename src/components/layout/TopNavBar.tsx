...
export default function TopNavBar() {
  ...
  const isVoter = typeof pathname === "string" && pathname.startsWith("/voter");
      ...
      <div className="flex items-center justify-end gap-2 sm:gap-3 min-w-0 w-full sm:w-auto">
        ...
        <div className="flex items-center gap-2 sm:gap-3 rounded-3xl bg-surface-container-highest border border-outline-variant px-2 sm:px-3 py-1.5 sm:py-2 shadow-sm min-w-0">
          <div className="min-w-0 text-right">
            <p className="truncate font-label-md text-on-surface text-sm sm:text-base">{isVoter ? "EGABO AARON" : "NATOZO PATIENCE MARTA"}</p>
            <p className="truncate text-xs sm:text-sm text-outline">{isVoter ? "Student" : "Super Admin"}</p>
          </div>
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-surface-container-lowest flex-shrink-0 border border-outline-variant">
            <img
              ...
              src={isVoter ? "/assets/Aaron.jpeg" : "https://lh3.googleusercontent.com/aida-public/AB6AXuApw3ezA29EEwzcevMJLty98oSo-J3VZ149Dj-L0-3qpDPk6IMJBJvOqhojCj6aWpP93zGCo-XmqP8nuiTIootJhWqjpjgBcqKEvmu82idg6VPaXFdHPabTLInG8UYll4E1jlNSoHmfPjcjjbkktRFuGVe-bFWgwS4YlGI6ssE2y91mVm9BddBxSXEuyNvbnF0diPjsrDgUQelPeXweN4HT1dux-FKcUrMZn2DRiNQj8rhxRG8olEDANS9Mc7pjGZKrHFjIA4uUbACguw"}
            />
          </div>
        </div>
      </div>
